...
/*
    ParametricEQ
    ============================================================
    EQ 3 bandes + coupe-bas/coupe-haut + bouton Air, pensé comme
    les mouvements de base d'un channel strip vocal, fréquence ET
    gain réglables par bande :
      ...
      - Air (bouton on/off, fixe ~12kHz/+2dB)     : shelf indépendant, s'ajoute
                                                     par-dessus le reste sans y toucher
...
public:
    ...
    void processStereo (float* left, float* right, int numSamples)
    {
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            l = airShelf[0].processSample (l); // gain=0dB si le knob Air est a 0 -> transparent
...
private:
    void updateCoefficients()
    {
        ...
        auto airCoeffs     = juce::dsp::IIR::Coefficients<float>::makeHighShelf (
            sampleRate, airFreq, 0.707f, juce::Decibels::decibelsToGain (airGainDb));
    ...
    float airGainDb = 0.0f; // 0 = knob Air a fond a gauche = transparent
    static constexpr float airFreq = 12000.0f;

    juce::dsp::IIR::Filter<float> lowCut1[2], lowCut2[2];
    juce::dsp::IIR::Filter<float> lowShelf[2], midBell[2], highShelf[2], airShelf[2];
